...
# Audio generation option - NOW DEFAULTS TO OFF
st.sidebar.markdown("---")
st.sidebar.markdown("### 🎧 Audio Generation")
enable_audio = st.sidebar.checkbox("Enable Audio Generation", value=False,
...
if enable_audio:
    ...
    st.sidebar.warning("⚠️ **Note:** Audio currently works best with 3-round debates. Longer debates may exceed free tier limits.")
...
if st.sidebar.button("🎯 Run Debate", type="primary"):
    ...
    # Check for ElevenLabs API key if audio is enabled
    ...
    with st.spinner(f"Running {rounds}-round debate..."):
        try:
            ...
            with col4:
                # Audio generation button - only if enabled and no errors
    ...
    6. **Enable audio generation** (Optional) - Generate MP3 audio of the debate
       - ⚠️ **Currently works best with 3-round debates**
       - Longer debates may exceed ElevenLabs free tier limits
    7. **Run the debate** - Watch the arguments unfold round by round
    8. **Export results** - Download transcripts and audio
    ...
    - **CON Voice**: Rachel (clear, professional female)
    - **Judge Voice**: Antoni (warm, articulate male)
    - **Format**: MP3 audio file
    - **Quality**: High-quality ElevenLabs voices
    - **Free Tier**: 10,000 characters/month
    - **⚠️ Best for 3-round debates** - Longer debates may exceed limits
    ...
    ✅ Generate MP3 audio with 3 distinct voices (best for 3 rounds)  
